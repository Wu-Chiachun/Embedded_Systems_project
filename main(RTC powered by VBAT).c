...
uint8_t DHT11_Read(){
	...
	LCD_DrawString(20, 100, "finish reading one data");
	return result;
}
...
//	if(first_in==1){
//		RTC_TimeTypeDef sTime_start = {0};
//		HAL_PWR_EnableBkUpAccess();
//		sTime_start.Seconds = HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR1);
//		sTime_start.Minutes = HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR2);
//		sTime_start.Hours = HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR3);
//		HAL_RTC_SetTime(&hrtc, &sTime_start, RTC_FORMAT_BCD);
...
//		HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR3, sTime.Hours);
//		HAL_PWR_DisableBkUpAccess();
//	}
	...
	if(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_13) == GPIO_PIN_SET){
		uint8_t sec=0x45, min=0x59, hr=0x23, date=0x31, year=0x23;
		// update RTC
		...
		sDate2.WeekDay = RTC_WEEKDAY_SUNDAY;
		sDate2.Month = RTC_MONTH_DECEMBER;
		sDate2.Date = date;
		sDate2.Year = year;
		...
		/*store value to backup register*/
		HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR2, (uint16_t)sDate2.Year);
		HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR3, (uint16_t)sDate2.Month);
		HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR4, (uint16_t)sDate2.Date);
		HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR5, (uint16_t)sDate2.WeekDay);
	}
